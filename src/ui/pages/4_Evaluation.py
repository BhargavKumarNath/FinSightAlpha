...
from components.data import EVAL_RESULTS, RAGAS_SUMMARY
...
if not RAGAS_SUMMARY or not EVAL_RESULTS:
    st.warning(
    ...
    # Metric explanation cards — real averages over RAGAS_SUMMARY['num_queries'] queries.
    col_l, col_r = st.columns(2, gap="large")

    with col_l:
        avg_faith = RAGAS_SUMMARY["avg_faithfulness"]
        faith_badge = badge_html("PASS ✓", GREEN) if avg_faith >= 0.7 else badge_html("BELOW THRESHOLD", RED)
        st.markdown(
        ...
        st.markdown(
            ...
            f"Threshold for PASS: ≥ 0.70 · Avg over {RAGAS_SUMMARY['num_queries']} real queries</div>"
            f"<div style='font-family:{DISPLAY};font-size:32px;font-weight:800;color:{PRIMARY};margin-bottom:8px;'>{avg_faith:.2f}</div>",
            unsafe_allow_html=True,
        )
        st.markdown(faith_badge, unsafe_allow_html=True)
        st.markdown("</div>", unsafe_allow_html=True)

    with col_r:
        avg_relev = RAGAS_SUMMARY["avg_answer_relevancy"]
        relev_badge = badge_html("PASS ✓", GREEN) if avg_relev >= 0.7 else badge_html("BELOW THRESHOLD", RED)
        st.markdown(
            f"<div style='background:{SURFACE};border:1px solid {BORDER};"
        ...
        st.markdown(
            ...
            f"<div style='font-family:{MONO};font-size:11px;color:{TEXT_MUTED};margin-bottom:12px;'>"
            f"Scale: 0.0 (irrelevant) → 1.0 (perfect) · Avg over {RAGAS_SUMMARY['num_queries']} real queries</div>"
            f"<div style='font-family:{DISPLAY};font-size:32px;font-weight:800;color:{ACCENT};margin-bottom:8px;'>{avg_relev:.2f}</div>",
            unsafe_allow_html=True,
        )
        st.markdown(relev_badge, unsafe_allow_html=True)
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    for i, r in enumerate(EVAL_RESULTS):
        ...
        row_passes = r["faith"] >= 0.7 and r["relev"] >= 0.7
        num_passed += int(row_passes)
        pass_badge = badge_html("PASS", GREEN) if row_passes else badge_html("REVIEW", RED)
    ...
    st.markdown(
        ...
        f"◆ Average Faithfulness: <b>{RAGAS_SUMMARY['avg_faithfulness']:.2f}</b> · "
        f"Average Relevancy: <b>{RAGAS_SUMMARY['avg_answer_relevancy']:.2f}</b> · "
        f"<b>{num_passed}/{len(EVAL_RESULTS)}</b> queries PASS both thresholds (≥ 0.70)</div>",
        unsafe_allow_html=True,
    )
    st.markdown("</div>", unsafe_allow_html=True)
...
st.markdown(
    ...
    f"<p style='font-size:12px;color:{TEXT_DIM};line-height:1.7;margin:10px 0 0;'>"
    f"Observed: in the current real evaluation run, 2 of 3 queries scored <code>answer_relevancy = 0.0</code> "
    f"(see the table above) — consistent with this <code>n=1</code> reduction degrading the reverse-question "
    f"generation AnswerRelevancy depends on. Not yet root-caused; recorded here rather than hidden.</p>"
    f"</div>",
    unsafe_allow_html=True,
)